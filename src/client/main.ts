...
import { createCameraController } from './game/camera-controller';
import {
  setAIDifficulty,
  setLatencyMs,
...
import { createConnectionManager } from './game/connection';
import { applyClientGameState } from './game/game-state-store';
import { createHudController } from './game/hud-controller';
...
import { createTurnTimerManager } from './game/timer';
import { createLocalGameTransport, type GameTransport } from './game/transport';
import { createTurnTelemetryTracker } from './game/turn-telemetry';
...
type MainNetworkDeps = Parameters<typeof handleServerMessageFromMain>[0];
...
const createGameClient = () => {
  ...
  let getMainNetworkDeps!: () => MainNetworkDeps;
  ...
  const handleInput = (event: InputEvent) => {
    if (ctx.state === 'playing_movementAnim') return;
  ...
  const handleMessage = (msg: S2C) => {
    handleServerMessageFromMain(getMainNetworkDeps(), msg, () =>
      replayController.onGameOverMessage(),
    );
  };

  const exitToMenu = () => exitToMenuFromMain(getMainNetworkDeps());

  const connection = createConnectionManager({
    getGameCode: () => ctx.gameCode,
    getGameState: () => ctx.gameState,
    getClientState: () => ctx.state,
    getStoredPlayerToken: (code) => sessionApi.getStoredPlayerToken(code),
  ...
  const createLocalTransport = (): GameTransport => {
    return createLocalGameTransport({
      ...
      startLocalGame: (scenario) =>
        startLocalGameFromMain(getMainNetworkDeps(), scenario),
    });
  };
  ...
  const joinGame = (code: string, playerToken: string | null = null) => {
    beginJoinGameFromMain(getMainNetworkDeps(), code, playerToken);
  };
  ...
  const handleUIEvent = (event: UIEvent) => {
    ...
    switch (plan.kind) {
      ...
      case 'startSinglePlayer':
        ...
        startLocalGameFromMain(getMainNetworkDeps(), plan.scenario);
        return;
      case 'joinGame':
