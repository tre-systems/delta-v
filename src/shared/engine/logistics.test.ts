...
const makeState = (ships: Ship[], overrides: Partial<GameState> = {}): GameState => ({
  gameId: 'test',
  scenario: 'test',
...
describe('processSurrender', () => {
  ...
  it('rejects surrender when logistics not enabled', () => {
    const ship = makeShip({ id: 's1', owner: 0 });
    const state = makeState([ship], { phase: 'astrogation', activePlayer: 0, scenarioRules: {} });
    const result = processSurrender(state, 0, ['s1']);
    expect('error' in result).toBe(true);
  });
  ...
  it('surrenders multiple ships', () => {
    ...
    const state = makeState([s1, s2], { phase: 'astrogation', activePlayer: 0 });
...
describe('getTransferEligiblePairs', () => {
  it('finds friendly ship pairs at same hex+velocity', () => {
    const source = makeShip({ id: 'tanker', type: 'tanker', owner: 0, fuel: 50 });
    const target = makeShip({ id: 'corvette', type: 'corvette', owner: 0, fuel: 5 });
  ...
  it('excludes pairs at different hexes', () => {
    ...
    const target = makeShip({ id: 's2', type: 'corvette', owner: 0, fuel: 5, position: { q: 6, r: 5 } });
    const state = makeState([source, target]);
  ...
  it('excludes pairs with different velocity', () => {
    const source = makeShip({ id: 's1', type: 'tanker', owner: 0, fuel: 50 });
    const target = makeShip({ id: 's2', type: 'corvette', owner: 0, fuel: 5, velocity: { dq: 2, dr: 0 } });
    const state = makeState([source, target]);
    expect(getTransferEligiblePairs(state, 0)).toHaveLength(0);
  });

  it('torch ships cannot transfer fuel', () => {
    const source = makeShip({ id: 'torch', type: 'torch', owner: 0, fuel: Infinity });
    const target = makeShip({ id: 'corvette', type: 'corvette', owner: 0, fuel: 5 });
    const state = makeState([source, target]);
    const pairs = getTransferEligiblePairs(state, 0);
  ...
  it('allows looting disabled enemy ships', () => {
    const enemy = makeShip({ id: 'enemy', type: 'frigate', owner: 1, fuel: 15, damage: { disabledTurns: 2 } });
    const friendly = makeShip({ id: 'friendly', type: 'corvette', owner: 0, fuel: 5 });
  ...
  it('allows looting surrendered enemy ships', () => {
    const enemy = makeShip({ id: 'enemy', type: 'frigate', owner: 1, fuel: 15, surrendered: true });
    const friendly = makeShip({ id: 'friendly', type: 'corvette', owner: 0, fuel: 5 });
    const state = makeState([enemy, friendly]);
    const pairs = getTransferEligiblePairs(state, 0);
    expect(pairs.length).toBe(1);
  ...
  it('blocks looting operational enemy ships', () => {
    const enemy = makeShip({ id: 'enemy', type: 'frigate', owner: 1, fuel: 15 });
    const friendly = makeShip({ id: 'friendly', type: 'corvette', owner: 0, fuel: 5 });
    const state = makeState([enemy, friendly]);
    expect(getTransferEligiblePairs(state, 0)).toHaveLength(0);
  });
  ...
  it('excludes destroyed ships', () => {
    const source = makeShip({ id: 's1', type: 'tanker', owner: 0, fuel: 50, destroyed: true });
...
describe('processLogistics', () => {
  it('transfers fuel between ships', () => {
    ...
    const target = makeShip({ id: 's2', type: 'corvette', owner: 0, fuel: 5 });
    const state = makeState([source, target]);
    const transfer: TransferOrder = { sourceShipId: 's1', targetShipId: 's2', transferType: 'fuel', amount: 10 };
  ...
  it('transfers cargo between ships', () => {
    const source = makeShip({ id: 's1', type: 'frigate', owner: 0, cargoUsed: 0 }); // cargo 40
    const target = makeShip({ id: 's2', type: 'corvette', owner: 0, cargoUsed: 3 }); // cargo 5
    const state = makeState([source, target]);
    const transfer: TransferOrder = { sourceShipId: 's1', targetShipId: 's2', transferType: 'cargo', amount: 2 };
    const result = processLogistics(state, 0, [transfer], map);
    expect('error' in result).toBe(false);
  ...
  it('rejects transfer exceeding target fuel capacity', () => {
    ...
    const state = makeState([source, target]);
    const transfer: TransferOrder = { sourceShipId: 's1', targetShipId: 's2', transferType: 'fuel', amount: 5 };
    const result = processLogistics(state, 0, [transfer], map);
    expect('error' in result).toBe(true);
  });
