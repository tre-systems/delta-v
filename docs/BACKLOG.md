...
**Still open:** a pass to ensure forced-colors does not flatten game-over semantic colors where we still need victory/defeat distinction; spot-check other screens that reuse `.menu-content` without `#menu` (already covered in `base.css` `.screen`).
