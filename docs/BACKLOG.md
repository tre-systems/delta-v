...
_No active items. The public-leaderboard arc shipped on 2026-04-17 — see commit [`14599d6`](https://github.com/tre-systems/delta-v/commit/14599d6) for the full landing (D1 `player` + `match_rating` tables, Glicko-2 helper, `POST /api/claim-name` / agent-token `claim.username`, matchmaker-paired rating writer, `GET /api/leaderboard`, `/leaderboard` page, provisional gating). Remaining leaderboard-adjacent items are all triggered Future features below (Turnstile, PoW, OpenClaw SKILL.md)._
