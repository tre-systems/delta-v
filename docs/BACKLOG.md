...
- **Grand Tour AI now completes more reliably, but seat balance is still skewed.** The 2026-04-22 refuel-recovery fix removed the old 499-turn checkpoint-race deadlocks, and the 2026-04-24 pass added safer checkpoint approach / refuel scoring plus base-brake landing support. A fresh `grandTour 60 -- --ci --seed 1` sweep improved from `0/60` P0 to `18/60` P0 with no crashes, but it still warns at `30.0%` P0 and includes `15` fleet-elimination resolutions instead of clean race finishes. Action: keep the shared-base refuel recovery, then finish rebalancing the per-home route / checkpoint policy so both seats complete the objective consistently without one side getting a free lane. The simulation harness now warns on gross objective-seat skew for Grand Tour; keep that warning green. **Files:** `src/shared/ai/common.ts`, `src/shared/ai/astrogation.ts`, `src/shared/movement.ts`, `scripts/simulate-ai.ts`
