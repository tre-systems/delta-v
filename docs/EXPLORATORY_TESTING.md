...
**Other sensory checks.** In the same preview tab: emulate `prefers-contrast: more` and `prefers-reduced-motion: reduce` via `preview_eval` by setting `document.documentElement.style.colorScheme` / forcing a class, or use DevTools rendering panel in a headed browser. Then tab-only through lobby + one astrogation phase. Cross-reference with [MANUAL_TEST_PLAN.md § Accessibility audit](./MANUAL_TEST_PLAN.md#accessibility-audit-dom).
...
- Confirm [SECURITY.md § Privacy (technical summary)](./SECURITY.md#privacy-technical-summary) still matches the client/server storage reality after any recovery-flow change.
...
|------|----------|-------|----------------------|
| 2026-05-02 | Codex | Local pre-push verification of the fleet-builder escape hatch and phase-banner overlap fixes; unit suite, lint/typecheck/build, and all-scenario + blockade scorecards clean. | 0 — Agent D items retired; Blockade randomized-start false-positive retired |
| 2026-05-02 | Codex | Live pass vs deployed hash `f49fcdfb`: public contracts healthy; surfaced narrow-portrait `#phaseAlert` overlap and Blockade randomized-start pressure. | 2 |
| 2026-05-02 | Codex | Live pass vs deployed hash `c713f124`: contracts and 8-scenario sweep clean; surfaced missing fleet-builder exit affordance and unreachable tutorial completion. | 2 |
| 2026-05-02 | Codex | Post-deploy pass vs hash `7c12a96a`: API and mobile checks clean; surfaced Fleet Action fuel-stall drift and rating/archive/R2 retention parity gaps. | 2 |
| 2026-05-02 | agent (Opus 4.7) | Post-Stream-A verification vs `7c12a96a`: migrations present, R20 audit clean, Lunar Evacuation confirmed hidden; reaffirmed Blockade balance, Evacuation P0 dominance, and Convoy briefing findings already on the backlog. | 0 |
| 2026-05-02 | agent (Opus 4.7) | Live pass vs `124bfbd0` (R1/R2/R7/R10/R14/R17/R20): surfaced Bi-Planetary and Blockade live objective desync, asymmetric-briefing seat mismatch, unreachable tutorial completion, match-archive noise, `/robots.txt` HEAD drift, and two error-shape inconsistencies — handed to user, no backlog edits. | 0 |
| 2026-04-26 | Codex | Post-`d13b219` deep pass across public surface, recovery flow, hosted MCP, all scenario launches, and responsive sweeps; filed sound-control overlay clickability. | 1 |
| 2026-04-26 | Codex | Follow-up fix pass: overlay chrome positioning keeps `#soundBtn` clickable on menu-like overlays; compact viewports rechecked, entry retired. | 0 |
| 2026-04-27 | agent (Opus 4.7) | First R20 D1/R2 storage audit: surfaced asteroid-hazard parity drift (fixed), tutorial funnel with zero completions (instrumented), scenario discovery gap, and Duel seat skew. | 2 |
| 2026-04-27 | agent (Opus 4.7) | Post-deploy verification (R1/R2/R18/R19, R20-lite) all clean; filed `/create` error-code inconsistency and missing official-bot offer telemetry. | 2 |
