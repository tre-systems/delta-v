...
- **Decided-game P0 win-rate outside [40, 60]** at 100+ games → balance issue. CI bounds vary per scenario (see `BALANCE_THRESHOLDS` in [scripts/simulate-ai.ts](../scripts/simulate-ai.ts)): biplanetary is deliberately wide at `[0.45, 0.85]`, fleetAction `[0.45, 0.80]`, duel/convoy/interplanetaryWar `[0.30, 0.70]`, evacuation `[0.35, 0.65]`, blockade `[0.25, 0.80]`. Tighter local thresholds catch drift earlier than CI.
...
# Rating ↔ archive parity. Rated games should keep an archive/replay
# record, or carry an explicit retired/hidden status so public surfaces
# do not imply a replay exists.
npx wrangler d1 execute delta-v-telemetry --remote --json --command \
  "SELECT COUNT(*) AS rating_without_archive
...
data-consistency finding even when the object exists. A non-zero
`rating_without_archive` count is also a data-consistency finding unless each
row has an explicit retired/hidden status and no public route links to the
missing replay.
...
|------|----------|-------|----------------------|
| 2026-05-02 | Codex | Local pre-push fix verification against Wrangler dev build cache `delta-v-76bb5d22`. Browser Use verified Fleet Action exposes `BACK` in `fleetBuilding` and returns to the menu without a purchase; Interplanetary War exits the same setup screen with Escape. New Playwright checks pass for fleet-builder escape and for zero `#phaseAlert` / ship-card overlap at 320 x 568, 360 x 640, and 375 x 812. Full unit suite: 200 files / 2694 tests passed. `npm run lint`, `npm run typecheck:all`, and `npm run build` passed. `npm run simulate -- blockade 60 --ci --quiet --json` passed with 39/60 Mars landings, 0 crashes, 0 invalid actions, and 0 fuel stalls. `npm run simulate -- all 60 --ci --quiet --json` passed CI stability with 0 crashes, 0 invalid actions, and 0 fuel stalls across every scenario; the only non-fatal warning remains hidden Lunar Evacuation P0 decided rate 78.3 %. Blockade randomized-start pressure is now documented as a non-production stress case because the shipped scenario uses `startingPlayer: 1`. | 0 — Agent D items retired; Blockade randomized-start false-positive retired |
| 2026-05-02 | Codex | Live exploratory pass against deployed `version.json` assets hash `f49fcdfb`. R1/R2/R18/R19 public contracts: home, health, version, agent docs, matches, leaderboard, sitemap, validation errors, and core HEAD parity are healthy; `/robots.txt` remains the known GET 200 / HEAD 404 outlier. `/.well-known/agent.json` and `POST /create` still expose Lunar Evacuation to engine/API clients, while the player-facing scenario picker correctly shows only 8 cards. R7 visible scenario sweep launched every visible Play-vs-AI scenario with 0 console/page errors; Lunar Evacuation failed only because the old harness still expected a hidden card. Convoy/Escape/Blockade per-seat briefing copy is now aligned for P0/P1. Fleet Action still enters `fleetBuilding` with only `LAUNCH FLEET` and `CLEAR`, no exit/back affordance. R10 mobile sweep found a real narrow-portrait HUD overlap: during phase-entry at 320 x 568 / 360 x 640 / 375 x 812, `#phaseAlert` covers `.ship-list` for the first seconds of play (320 x 568 measured 8,813 px overlap). Multiplayer replacement probe passed in production (`SESSION_REPLACED`, close 1000 in 191 ms). R13 sanitized Worker tail during probes captured 105 chunks, all `outcome: ok`, 0 exceptions; `/telemetry` 204s dominated the sample. R20 Cloudflare: `events` fresh during the pass, 49 public archive rows, 0 one-turn rows, 0 <=2-turn rows, 0 null outcomes, 0 unretired rating/archive orphans, 11 historic retired rows marked `pre_audit_cleanup`, and 0 raw player-key telemetry rows after the deploy timestamp. Public `/api/matches` displays callsigns for every visible archive row despite legacy null snapshot columns. R2 sample objects for `37TR2-m1`, `E5MKM-m1`, and `H8TL9-m1` exist at 15-18 KB; sampled legacy top-level `completedAt` still drifts 5-6 minutes after D1/final-event time, with no fresh post-fix archive yet to prove the new idempotent path in production. R16 `all 60 --ci --quiet --json` passed stability with 0 crashes, 0 invalid actions, and 0 fuel stalls; warnings remain for Lunar Evacuation non-randomized P0 dominance (81.7 %, avg 2.02 turns). Focused `fleetAction 40` is clean (0 stalls/timeouts). Focused `blockade 80 --randomize-start` reopened Agent B: 47.5 % Mars landings, P0 decided 72.5 %, 9 fuel stalls. | 2 — Blockade randomized-start pressure; phone phase-banner overlap |
| 2026-05-02 | Codex | Live exploratory pass against deployed `version.json` assets hash `c713f124` after Agent B deployed. R1/R2/R18/R19 public contracts: core pages and JSON endpoints are healthy; validation errors are structured; `/robots.txt` remains the known GET 200 / HEAD 404 outlier. In-app browser reload had 0 console warnings/errors. R7 visible scenario sweep: 8 visible Play-vs-AI scenarios, Lunar Evacuation hidden; Bi-Planetary turn-1 keyboard burn + Enter works; Blockade Runner P0, Escape P0, and Convoy P1 briefing/objective copy aligned; Fleet Action and Interplanetary War both enter fleet building with no visible Exit/Back and Escape does not leave the screen. `/matches`, `/leaderboard`, and archived replay `37TR2-m1` render without console errors; Reyes and Kepler remain the top visible leaderboard rows. R16 scorecards: `all 60 --ci --quiet --json` passed with 0 crashes, 0 invalid actions, and 0 fuel stalls; the only warning was Lunar Evacuation P0 decided 71.7%. Focused runs: Bi-Planetary 30/30 landings, Blockade 47/60 Mars landings on seed `-403487708`, Evacuation P0 decided 83.3% / avg 2.03 turns. R20 Cloudflare: `events` fresh at 2026-05-02T10:40:50Z; `match_archive` still has 49 public rows, newest 2026-04-24; 0 one-turn disconnect rows, 0 <=2-turn rows, 0 unretired rating/archive orphans, and no raw player-key telemetry after 10:00Z. Live archived Bi-Planetary and Blockade rows still show 0 landing outcomes, but they predate the current scenario/AI fixes. Recent R2 objects exist (15-18 KB); sampled legacy top-level `completedAt` values still drift 5-6 minutes after D1/gameOver timestamps, with no fresh post-fix archive yet to prove the new path in production. R13 sanitized tail saw 89 request chunks, all `outcome: ok`, 0 exceptions. | 2 — fleet-builder escape hatch; tutorial completion reachability |
| 2026-05-02 | Codex | Post-deploy live exploratory pass against `version.json` hash `7c12a96a` immediately after deployment completed. R1/R2/R18/R19 public API probes: home, listings, health, agent docs, and validation are healthy; `/api/matches?status=definitely_bad` now returns structured `invalid_query`; `/robots.txt` still has GET 200 / HEAD 404. In-app browser reload showed no console warnings/errors. R7/R10 browser sweep: all 8 visible Play-vs-AI scenarios launch; Lunar Evacuation remains hidden; Bi-Planetary keyboard burn + Enter advances to opponent turn; mobile overlap/click-reachability checks at 320×568, 375×812, 812×375, 641×800, and 761×900 found no overlap or click-stealing. Convoy P1 still has the fixed escort-mission description beside objective `⬡ Destroy all enemies`; Escape P1 reads clearly. R16 `npm run simulate -- all 60 --ci --quiet --json`: engine stable with 0 crashes/invalid actions, Convoy healthy at 85% objective share and 0 fuel stalls, but Fleet Action produced 442 fuel stalls (7.37/game) and 2 timeouts; focused `fleetAction 20 --seed -403487708 --capture-failure-kind fuelStall` reproduced 442 stalls (22.1/game). Blockade Runner remains attrition-heavy: 8/60 landings in all-scenario sweep and 6/40 in focused capture. R20 Cloudflare: `events` fresh at 2026-05-02T09:08:59Z, `match_archive` has 49 clean public rows but newest archive is 2026-04-24; public match noise is now 0/49 for one-turn disconnects and 0/49 for <=2-turn rows. Recent archived rows have R2 objects, but `match_rating LEFT JOIN match_archive` reports 11 rating rows without archive/R2; examples `BCFV9-m1` and `3PJYX-m1` have `rating_applied` + old `archived_replay_fetch_succeeded` events but their R2 objects are missing. R13 sanitized tail during live API probes saw 5 log chunks and 0 exceptions. | 2 — Fleet Action fuel-stall drift; rating/archive/R2 retention parity |
...
| 2026-04-27 | agent (Opus 4.7) | Post-deploy verification pass following the help-screen / matches-page / button-icon ships and the asteroid-parity + tutorial-funnel telemetry: R1 surface scan (17 paths — `/how-to-play` and `/metrics` 404 as designed, all other documented paths 200), R2 validation (`/api/matches`, `/api/leaderboard` reject limit > cap with structured `invalid_query`; `/create` rejects empty/fake-scenario/extras correctly; `/api/agent-token` rejects wrong prefix), R18 HEAD parity (`/api/matches`, `/api/leaderboard`, `/healthz`, `/api/leaderboard/me`, `/version.json`, `/` all `GET == HEAD`), R19 error-shape consistency (every probed route returns unified `{ok:false, error, message}`), R20 lite (D1 row counts + freshness — events fresh, `projection_parity_mismatch` 0 in past 24h ✓ confirms shipped fix not regressing). Surfaced: (1) `/create` with `{}` returns `invalid_payload` "only supports scenario" while no-body returns `missing_scenario` — same root cause, different error codes (P3); (2) zero `quick_match_official_bot_accepted` events in past 65 h despite 25 attempts and one user waiting 133 s — can't tell if offer is invisible or just not compelling, instrument `quick_match_official_bot_offered` to disambiguate (P2). Verified `tutorial_step_shown` ships in deployed bundle but no traffic since deploy, so funnel signal hasn't accumulated yet. | 2 |
