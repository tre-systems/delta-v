...
Setting up a local development environment takes five steps.

First, switch to the Node version pinned by the project's version file — the continuous integration workflow uses the same version. Second, install the project's dependencies using the package manager. Third, install Playwright's Chromium browser, which is used by pre-commit hooks and the continuous integration suite for browser smoke tests. Fourth, start the development server. Fifth, open a browser to the local server address, create a game in one tab, copy the room link or five-character code, and join from a second tab.

### CLI Commands

Several npm scripts are available. The dev script starts local development using the Wrangler bundler and esbuild. The build script produces the client bundle. The lint script runs Biome for linting and format checking. The typecheck script checks types in the source directory, and the typecheck-all variant also covers scripts, end-to-end tests, and configuration files.

For testing, the test script runs all unit tests using Vitest. The test coverage script runs tests with coverage reporting and enforces thresholds on the shared engine code. The end-to-end test script runs Playwright browser smoke tests against a local Wrangler server. The accessibility end-to-end test script adds an axe accessibility baseline to that suite. A headed variant of the smoke suite runs the same tests with a visible browser. The watch script runs Vitest in watch mode.

The verify script runs a full pre-release sweep covering lint, type checking, coverage, build, end-to-end tests, accessibility, and simulation. The simulate script runs headless AI-versus-AI matches; you pass a scenario name and iteration count as arguments. The load test script runs a WebSocket load and chaos harness; you pass the number of games and concurrency level as options. Finally, the deploy script deploys to Cloudflare Workers.
