...
Hands-on verification for release candidates and significant changes. Each section is self-contained, so run only the ones relevant to what changed.

## Release gate

Any of these is a blocker: a new player cannot start Bi-Planetary and finish turn one using only in-game guidance; the current player, selected ship, objective, or next required action is unclear at any point; multiplayer create, join, reconnect, chat, rematch, or disconnect resolution is flaky; mobile and touch has blocked actions, overlapping heads-up display, or unreadable text; Progressive Web App offline single-player is broken; or a twenty-to-thirty minute session shows serious stutter, dropped input, stale interface, or unclear win or loss messaging.
...
1. Open the app. The menu shows the title, difficulty, and play options.
2. Pick Easy, then Bi-Planetary. The status indicates first-burn fuel cost and free takeoff.
3. Click a direction arrow or press one of the number keys one through six, or zero to clear. A course preview and fuel-cost label appear.
4. Click Confirm or press Enter. The ship animates and the heads-up display updates.
5. The AI takes its turn, then yours resumes.

Pass: takeoff, movement, and AI response all work.

### 1a. First-time UX (fresh profile, 5 min)

Start from the menu without opening any external documentation. Within the first ten seconds it should be obvious which ship is yours, what the goal is, and what the next action is. Finish turn one without guessing what Confirm will do. Make and recover from a deliberate mistake — such as a wrong burn or wrong selection — using only in-game affordances like deselect, undo, or help. After the AI turn, within three seconds you should know the objective, the selected ship, the next action, and why the last event happened. Reach turn three without any external explanation.
...
Each scenario has its rules fully specified in the game specification document. Quick verification points follow. Bi-Planetary: land on the opposite planet. Escape: hidden fugitive ship, only nukes available with no mines or torpedoes, planetary defense disabled, and a moral victory is awarded on Enforcer disable. Convoy: liner, tanker, and frigate escort with logistics enabled; land the liner on Venus with colonists. Lunar Evacuation: passenger rescue enabled and the win requires passengers aboard. Duel: last ship standing. Blockade Runner: packet with a head-start velocity, land on Mars. Fleet Action: four-hundred credit fleet build, covered in section eight. Interplanetary War: eight-hundred-fifty credit fleet build with logistics and longer play. Grand Tour: combat disabled, shared bases, visit eight checkpoints and return home.
...
The budget is shown, ship cards display stats and cost, and over-budget ships are greyed out. Removing a ship clears it from the selection; the clear button resets the entire fleet; Launch Fleet starts the match. The AI also builds its own fleet.
...
Install the app if the browser supports it; the launched shell should look correct. Online local AI play should work in the shell. To test offline, use browser developer tools to simulate an offline or airplane-mode state: the app shell should still load; start a single-player AI match; play at least three turns; multiplayer should fail clearly without hanging. Re-enabling the network should allow online play to recover after a retry or reload.
...
For human claims, a fresh browser profile should be able to claim a unique username through the home-screen callsign field, which is backed by the claim-name endpoint. Re-claiming the same username from a different player key should return a 409 conflict response.
...
The per-player rank lookup endpoint should return the username, rank, rating, and related fields when the player key is known, or a 404 when the key is unclaimed.

## 22. Edge cases / regression grab-bag

1. Zero fuel: the ship drifts at current velocity with no burn options; gravity, resupply, and map-exit rules still apply.
2. Map exit: a final course that goes off the edge of the map results in elimination.
3. Nuke clears asteroid: an asteroid hex becomes clear space after a nuke detonates.
4. Destroyed-ship cleanup: no ghost highlight or selection; clicking a wreck hex does not produce an error.
5. Empty combat phase: a skip-combat option is available, or the phase auto-skips.
6. Stacked ships: clicking cycles through all ships in the hex.
7. Turn timer: after two minutes of idle time the turn times out; a warning fires at thirty seconds.
8. Rematch: the same scenario and opponent restart with a fully cleared state.

---

## Automated checks

These checks run in continuous integration and do not replace manual experience checks. The full local release gate runs lint, type-checking, coverage, build, end-to-end tests, accessibility checks, and simulation together. A separate command runs unit, property, and regression tests. A thin browser smoke test uses Playwright. A Playwright plus axe accessibility baseline test checks the accessibility baseline. The simulation command sweeps all scenarios for engine stability and balance, using forty iterations locally and sixty in pre-commit and continuous integration. Code-style and type-safety checks run lint and type-checking across all source files.

Playwright is intentionally small. Scenario walk-throughs and deep rule validation belong in the unit and simulation test layers, not in Playwright.
