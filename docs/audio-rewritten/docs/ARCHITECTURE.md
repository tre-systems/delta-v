...
- Public leaderboard: shipped. Glicko-2 ratings are written after each rated match by the rating-writer module. The player table tracks each claimed username and the current rating; the player-recovery table stores one-way hashes for human callsign recovery codes; and the match-rating table holds per-match before-and-after rating snapshots. Humans claim a username through the claim-name endpoint; agents claim through the agent-token endpoint with an optional claim body. Human players can issue, restore, or revoke a callsign recovery code through the player-recovery endpoints, and raw recovery codes are not stored. Provisional players are hidden from the default view until their rating deviation shrinks and they meet a distinct-opponents threshold. The public page is the leaderboard path; the API consists of the main leaderboard query and a per-player rank lookup.
...
Bundle baseline, re-measured on the thirtieth of April 2026 from the current minified client bundle: the raw size is approximately four hundred thirty-one kilobytes and the gzip-compressed size is approximately one hundred twenty-nine kilobytes. Update these figures after large renderer or dependency changes.
...
Database migrations: treat as forward-only unless Cloudflare backup and restore is used. Rolling back means redeploying the previous Worker with a compatible schema — there is no automatic down-migration. The migrations directory currently holds six files: creating the events table, creating the match archive, adding match-archive listing support, adding the leaderboard schema, adding the official-bot tagging column to the match archive, and adding hashed human callsign recovery codes.
