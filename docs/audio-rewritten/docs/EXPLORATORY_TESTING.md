...
The Playwright preview vantage drives the local development server in a headless Chromium that honors viewport resize down to three hundred twenty pixels and respects preference-based media emulation through its evaluate tool. It is the only reliable way for an agent to exercise the responsive breakpoints at seven hundred sixty, six hundred forty, and four hundred twenty pixels. You start it with the preview-start tool, which spawns the dev watch script, then use the preview resize, snapshot, screenshot, inspect, evaluate, and click tools. This vantage is essential for the mobile layout recipe.

The Local Agent MCP vantage uses standard input and output. It drives a seat programmatically and lets you inspect the raw observation, candidates, and tactical hints. You start it with the local MCP run script, as described in the Delta-V MCP reference.
...
Viewport matrix. At minimum, step through every CSS breakpoint boundary and one real device on each side — missing the boundary misses bugs that live only in the single-pixel band between rules in the responsive stylesheet.
...
Recent regression hotspots worth extra scrutiny — a quick grep of the git log for mobile, overlap, heads-up display, and floating keywords enumerates them: the floating exit, chat, and replay-exit overlap, which has been fixed and re-regressed repeatedly. The heads-up-display bottom bar versus utility buttons on narrow and short viewports. The ship list overlapping the minimap at the six-forty-one to seven-sixty band. The game-over edge-to-edge treatment at four-twenty and below. Safe-area insets for notched devices in landscape. And the installed progressive-web-app shell, where there is no URL bar — verify the inset math separately from the in-browser view.
...
Only testing at one so-called mobile viewport, typically three-seventy-five by eight-twelve. Delta-V has breakpoints at seven-sixty, six-forty, and four-twenty pixel widths, plus a five-sixty short-height rule, and a narrow-and-short combination. Three-seventy-five by eight-twelve exercises only a subset. The mobile-sweep matrix includes one-pixel boundary viewports specifically because overlap bugs hide in the single-pixel band between media rules. Skipping the boundary means shipping the band.
...
Also on April twenty-fourth, a multiplayer connectivity probe against local development covered create validation, join preflight, quick-match enqueue and polling, idempotent same-player tickets, paired WebSocket flow, reconnect with the stored player token, spectator mode, and protocol-frame abuse — all returning the invalid-input error code — plus a rate-limit close with a WebSocket close code of ten-oh-eight and a reason string. Core flows passed. One multiplayer-connectivity-diagnostics entry was filed under gameplay UX, covering six client-side error-ergonomics gaps: handshake-rejection opacity, the generic invalid-input code, quick-match error-field naming, silent seat claim via the spectator query parameter, join empty-body four-oh-fours, and a same-token second-socket zombie.

Also on April twenty-fourth, a Codex live-site exploratory pass ran public API scans and validation against the match-list, leaderboard, and metrics endpoints and against bad join and replay identifiers, plus an agent-metadata consistency sweep, a Play-versus-AI Duel launch, a compact live mobile matrix across three-twenty by five-sixty-eight, three-seventy-five by eight-twelve, eight-twelve by three-seventy-five, six-forty-one by eight-hundred, and seven-fifty-nine by nine-hundred, and a client-state audit. Core public endpoints and the local AI launch passed; a priority-three tiny-phone utility-button overlap was filed.
