...
function chapterTitleFromMarkdown(markdown, fallback) {
...
function niceAssetName(relativePath) {
...
const chapterIdByFile = new Map(chapters.map((chapter) => [displayPath(chapter.file), chapter.chapterId]));
...
function resolveHref(rawHref, sourceFile, currentChapterId) {
...
function resolveImage(rawHref, sourceFile) {
  if (!rawHref) return rawHref;
...
function createRenderer(chapter) {
  ...
  renderer.link = function link(token) {
    const href = resolveHref(token.href, chapter.file, chapter.chapterId);
  ...
  renderer.image = function image(token) {
    const src = resolveImage(token.href, chapter.file);
    const title = token.title ? ` title="${escapeHtml(token.title)}"` : "";
...
function chapterHtml(chapter) {
  const renderer = createRenderer(chapter);
...
function partBreakHtml(part, partNumber) {
  return `
  ...
  const mapFigure = `
    <section class="appendix-section">
      ...
      <figure class="full-figure">
        <img class="doc-image" src="${escapeHtml(fileUrl("docs/map.png"))}" alt="Delta-V strategic map" />
    ...
    .map(
      (asset) => `
        <figure class="gallery-item">
          <img class="doc-image" src="${escapeHtml(fileUrl(asset))}" alt="${escapeHtml(niceAssetName(asset))}" />
...
for (const [index, part] of parts.entries()) {
  ...
  for (const chapter of chapters.filter((item) => item.partTitle === part.title)) {
    chapterSections.push(chapterHtml(chapter));
  }
}
...
<html lang="en">
  ...
  <body>
    <section class="cover">
      ...
      <div>
        <img src="${escapeHtml(fileUrl("screenshot.png"))}" alt="Delta-V gameplay screenshot" />
      </div>
    </section>
